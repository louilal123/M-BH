...
if ($result->num_rows > 0) {
    ...
    while ($room = $result->fetch_assoc()) {
      ...
      // ... [previous code remains the same until the room card section] ...
...
<div class="h-64 overflow-hidden relative group">
    ...
    <div class="absolute top-4 right-4 bg-green-500 text-white text-xs font-semibold px-3 py-1 rounded-full shadow-lg backdrop-blur-sm bg-opacity-90">
        Available
    </div>
    <div class="absolute bottom-4 right-4 flex items-center gap-2">
       
        <span class="bg-white/90 dark:bg-gray-900/90 backdrop-blur-sm rounded-full px-3 py-1 shadow-sm text-sm">
            ' . ($imageCount > 0 ? $imageCount . ' Photos' : 'No Gallery') . '
        </span> 
        <button onclick="showRoomGallery(' . $roomId . ')" class="flex items-center bg-white/90 dark:bg-gray-900/90 backdrop-blur-sm rounded-full px-3 py-1 shadow-sm text-sm">
...
echo '</div>
    ...
    <a href="room_details.php?id=' . $roomId . '" class="block text-center bg-primary hover:bg-primary-dark text-white py-2 rounded-md transition duration-300">
        Book Now
    </a>
</div>
</div>';
